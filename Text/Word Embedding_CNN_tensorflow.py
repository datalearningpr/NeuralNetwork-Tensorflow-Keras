...
# this version is bit different from the Keras one, but basic is the same
# just the CONVs are using 3 different sizes
#############################################################################
...
class CNN_comment():
    def __init__(self):
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                conv = tf.layers.conv2d(inputs = self.embedded_chars_expanded,
                ...
                m = tf.layers.max_pooling2d(inputs = conv,
                    pool_size = (sequence_len - filter_size + 1, 1),
                    strides = (1, 1),
                    name = "max_pool-%s" % filter_size
                )

                pooled_outputs.append(m)

        pooled_outputs_concat = tf.concat(pooled_outputs, 3)

        f = tf.layers.flatten(inputs = pooled_outputs_concat,
            name = "f"
        )

        d = tf.nn.dropout(f, 0.8)

        self.pred = tf.layers.dense(inputs=f, units=3, 
                                    name = "pred")
    ...
    def save_model(self):
        self.saver.save(self.sess, './CNN_text_model.ckpt')

    def load_model(self):
        self.saver.restore(self.sess, './CNN_text_model.ckpt')
...
pred = model.sess.run(model.pred, feed_dict={model.x_input: np.asmatrix(test_x)})
