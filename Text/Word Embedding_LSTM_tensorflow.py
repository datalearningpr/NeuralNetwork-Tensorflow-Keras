...
class LSTM_comment():
    def __init__(self):
        ...
        lstm_cell_dropout = tf.nn.rnn_cell.DropoutWrapper(lstm_cell, 0.8, 0.8)
        outputs, states = tf.nn.dynamic_rnn(lstm_cell_dropout, self.embedded_chars, dtype = tf.float32)
      

        self.pred = tf.layers.dense(inputs=tf.transpose(outputs, [1, 0, 2])[-1], units=3, 
                                    name = "pred")
    ...
    def save_model(self):
        self.saver.save(self.sess, './LSTM_text_model.ckpt')

    def load_model(self):
        self.saver.restore(self.sess, './LSTM_text_model.ckpt')
