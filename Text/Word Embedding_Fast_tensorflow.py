...
sequence_len = 400      # to make each sentence to same length
...
#############################################################################
# Fast model with word embedding
# The model is fast because it only use the output of embedding
# with Global Average Pooling without any hidden layer
# output is surprising good
#############################################################################
...
class Fast_comment():
    def __init__(self):
        ...
        a = tf.layers.average_pooling1d(self.embedded_chars, pool_size = sequence_len, strides = 1)

        f = tf.layers.flatten(a, name = "faltten")
    ...
    def save_model(self):
        self.saver.save(self.sess, './Fast_text_model.ckpt')

    def load_model(self):
        self.saver.restore(self.sess, './Fast_text_model.ckpt')
